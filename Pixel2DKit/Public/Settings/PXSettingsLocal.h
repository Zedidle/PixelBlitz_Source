...
USTRUCT()
struct FPXScalabilitySnapshot
{
	GENERATED_BODY()

	FPXScalabilitySnapshot();
...
UCLASS()
class PIXEL2DKIT_API UPXSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EPXStatDisplayMode GetPerfStatDisplayState(EPXDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EPXDisplayablePerformanceStat Stat, EPXStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UPXSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EPXDisplayablePerformanceStat, EPXStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FPXScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FPXScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UPXSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UPXSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	...
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UPXLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingToDefault(const FName MappingName, UPXLocalPlayer* LocalPlayer);
	...
	void ResetKeybindingsToDefault(UPXLocalPlayer* LocalPlayer);
