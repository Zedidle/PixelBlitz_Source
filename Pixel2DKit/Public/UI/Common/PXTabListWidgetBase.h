...
#include "PXTabListWidgetBase.generated.h"
...
class UPXTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IPXTabButtonInterface
{
	GENERATED_BODY()
...
class PIXEL2DKIT_API UPXTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
		
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FPXTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FPXTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FPXTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UPXTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FPXTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FPXTabDescriptor> PendingTabLabelInfoMap;
};
