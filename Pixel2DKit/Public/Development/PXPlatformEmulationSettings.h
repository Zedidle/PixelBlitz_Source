...
#include "PXPlatformEmulationSettings.generated.h"

struct FPropertyChangedEvent;

UCLASS(config=EditorPerProjectUserSettings, MinimalAPI)
class UPXPlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()
	
public:
	UPXPlatformEmulationSettings();
...
private:
	...
	// The base device profile to pretend we are using when emulating device-specific device profiles applied from UPXSettingsLocal
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="PX.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="PX.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(InlineEditConditionToggle, ConsoleVariable="PX.Settings.ApplyDeviceProfilesInPIE"))
	bool bApplyDeviceProfilesInPIE = false;

#if WITH_EDITOR
public:
	// Called by the editor engine to let us pop reminder notifications when cheats are active
	PIXEL2DKIT_API void OnPlayInEditorStarted() const;
