...
#include "Settings/PXWorldSettings.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PXWorldSettings)

APXWorldSettings::APXWorldSettings(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FPrimaryAssetId APXWorldSettings::GetDefaultGameplayExperience() const
{
...
#if WITH_EDITOR
void APXWorldSettings::CheckForErrors()
{
	...
	for (TActorIterator<APlayerStart> PlayerStartIt(GetWorld()); PlayerStartIt; ++PlayerStartIt)
	{
		...
		if (IsValid(PlayerStart) && PlayerStart->GetClass() == APlayerStart::StaticClass())
		{
			MapCheck.Warning()
				...
				->AddToken(FTextToken::Create(FText::FromString("is a normal APlayerStart, replace with APXPlayerStart.")));
		}
	}
