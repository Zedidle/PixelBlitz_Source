...
#include "Settings/PXExperienceDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PXExperienceDefinition)

#define LOCTEXT_NAMESPACE "PXSystem"

UPXExperienceDefinition::UPXExperienceDefinition()
{
}

#if WITH_EDITOR
EDataValidationResult UPXExperienceDefinition::IsDataValid(FDataValidationContext& Context) const
{
	...
	if (!GetClass()->IsNative())
	{
		...
		{
			Context.AddError(FText::Format(LOCTEXT("ExperienceInheritenceIsUnsupported", "该蓝图子类的玩法目前不支持。 Parent class was {0} but should be {1}."), 
...
void UPXExperienceDefinition::UpdateAssetBundleData()
{
